...
import { registerTab, buildDayNav, taskCard, weekOff } from "../router.js";
import { getTasks, getGoals, getProjects, getWeekGoals, saveWeekGoal, dstr, esc } from "../db.js";
...
export async function renderPlan() {
  ...
  sb.innerHTML = `
    ...
    ${goals.length ? goals.map((g,i) => `
      ...
      </div>`).join("") : '<p style="font-size:11px;color:var(--tx-l)">Целей нет</p>'}
    <div style="margin-top:14px">
      <div class="sb-sec">Цель недели</div>
      <div class="wg-block"><div class="wg-val" contenteditable id="wg-w"
        data-ph="Введите цель..."
        onblur="window._saveWG('week',this.textContent,'${wg.id||""}')">${esc(wg.week||"")}</div></div>
      <div class="sb-sec">Цель на месяц</div>
      <div class="wg-block"><div class="wg-val" contenteditable id="wg-m"
        data-ph="Введите цель..."
        onblur="window._saveWG('month',this.textContent,'${wg.id||""}')">${esc(wg.month||"")}</div></div>
      <div class="sb-sec">Цель на год</div>
      <div class="wg-block"><div class="wg-val" contenteditable id="wg-y"
        data-ph="Введите цель..."
        onblur="window._saveWG('year',this.textContent,'${wg.id||""}')">${esc(wg.year||"")}</div></div>
    </div>`;
  ...
  const items = showAll ? tasks : tasks.filter(t => t.date === dstr(planDate));
  const open  = items.filter(t => !t.done).sort((a,b) => (a.deadline||"") > (b.deadline||"") ? 1 : -1);
