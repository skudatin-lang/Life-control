...
// Генерация повторяющихся задач (создание реальных копий на период)
export async function generateRecurringInstances(parentTask, startDate, endDate) {
  ...
  while (current <= end) {
    let shouldAdd = false;
    const day = current.getDay(); // 0 воскр, 1 понед...
    switch(type) {
      case 'daily': shouldAdd = true; break;
      case 'weekly': shouldAdd = true; break; // интервал в неделях
    ...
    if (shouldAdd) {
      ...
      newTask.recurrence = null; // дочерние не повторяются
      newTask.done = false;
      newTask.createdAt = ss();
      instances.push(newTask);
    }
    // увеличиваем дату в зависимости от типа
  ...
  // удаляем старые экземпляры этого родителя
...
export const addTask = async data => {
  ...
  // если задача повторяющаяся и не дочерняя, генерируем экземпляры
...
export const updateTask = async (id, data) => {
  ...
  // если изменилось повторение – перегенерировать
  if (data.recurrence) {
...
export const deleteTask = async id => {
  // удаляем также дочерние повторяющиеся
...
export const toggleTask = async id => {
  ...
  if (t) {
    await updateDoc(ud("tasks", id), { done: !t.done });
    // если это родительская повторяющаяся, то переключаем все дочерние? не будем
  }
};

// остальные CRUD (goals, projects, ideas, diary, templates, weekgoals, mmpos, stats) остаются без изменений
// ... (копируем из предыдущей версии)
// Для краткости оставлю заглушки, но в реальном коде они должны быть как в предыдущем ответе
...
export const getStats = async () => {
  ...
  return { tasks, goals, ideas, diary, todayOpen: tt.filter(t=>!t.done).length, todayDone: tt.filter(t=>t.done).length, overdue: tasks.filter(t=>!t.done && isOv(t.deadline)).length, todayTasks: tt };
};